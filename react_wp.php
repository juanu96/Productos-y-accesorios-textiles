<?php
/*
Plugin Name: textiles Plugin
Description: Use the [textiles] shortcode to display the plugin
Version: 0.0.1
...
class textiles
{
    ...
    function add_type_attribute_front($tag, $handle, $src)
    {
        ...
        if ($handle  === 'textiles-plugin-dev' || $handle  === 'textiles-plugin-prod') {
            $tag = '<script type="module" src="' . esc_url($src) . '"></script>';
            return $tag;
        } else if ($handle  === 'textiles') {
            $tag = '<script type="module" src="' . esc_url($src) . '"></script>';
            return $tag;
        }
    ...
    function plugin_shortcode($atts)
    {
         ...
         */
        $tag = '<div id="textiles" class="textiles"></div>';
    ...
    function api_init()
    {
        register_rest_route('textiles/v1', '/user/', [
            'methods'   => 'GET',
    ...
    function init()
    {
        add_shortcode('textiles', [$this, 'plugin_shortcode']);
